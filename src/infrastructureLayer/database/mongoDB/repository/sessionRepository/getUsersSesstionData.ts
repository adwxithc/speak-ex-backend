    ...
    const sessionStats = (await sessionModel.aggregate([
        ...
        },
        {
            $project: {
                helpingSessions: {
                    $ifNull: [
                        { $arrayElemAt: ['$totalHelpingSessions.count', 0] },
                        0,
                    ],
                },
                learningSessions: {
                    $ifNull: [
                        { $arrayElemAt: ['$totalLearningSessions.count', 0] },
                        0,
                    ],
                },
                rating: {
                    $ifNull: [{ $arrayElemAt: ['$rating.avgRating', 0] }, null],
                },
                avgHelpingSessionsPerMonth: {
                    $ifNull: [
                        {
                            $arrayElemAt: [
                                '$helpingSessionsPerMonth.avgHelpingSessionsPerMonth',
                                0,
                            ],
                        },
                        0,
                ...
                avgLearningSessionsPerMonth: {
                    $ifNull: [
                        {
                            $arrayElemAt: [
                                '$learningSessionsPerMonth.avgLearningSessionsPerMonth',
                                0,
                            ],
                        },
                        0,
