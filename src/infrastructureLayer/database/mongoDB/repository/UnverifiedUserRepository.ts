...
export class UnverifiedUserRepository implements IUnverifiedUserRepository{
    ...
    async findUser(email:string):Promise<IUnverifiedUser | null> {
        ...
        let isUserExist = await UnverifiedUserModel.findOne({email})
    ...
    async createUserCollection(newUser: IUnverifiedUser): Promise<IUnverifiedUser> {
    ...
    async findUserAndDelete(email: string): Promise<boolean | IUnverifiedUser | null> {

        let result = await UnverifiedUserModel.findOneAndDelete({email})
        return result

    }

    async findUserWithOTP(email: string, otpFromUser: string): Promise<IUnverifiedUser | null> {
